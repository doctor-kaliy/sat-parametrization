...
pool = ThreadPool(processes=n_processes * 2)
...
def distribute_elements(n, r):
    num_buckets = n // r
    bucket_assignments = list([[] for i in range(num_buckets)])
    
    elements = list(range(n))
    np.random.shuffle(elements)
    
    for i in range(num_buckets * r):
        bucket_assignments[i // r].append(elements[i]) 
    
    bucket = 0
    for i in range(num_buckets * r, n):
        bucket_assignments[bucket].append(elements[i])
        bucket += 1

    return bucket_assignments

def neighbours_h_1(v, neighbours):
...
def bit_merge(bucket_assignments, v):
    res = []
    for bucket in bucket_assignments:
        n = 0
        for i in range(len(bucket)):
            n = assign(n, i, get(v, bucket[i]))
        res.append(n)
    return res

def bit_unmerge(bucket_assignments, v):
    res = 0
    for bucket, e in zip(bucket_assignments, v):
        for i in range(len(bucket)):
            res = assign(res, bucket[i], get(e, i))
    return res
...
def hill_climb(v, neighbours, f, steps, ba):
    global pool
    global n_processes
    nv = None
    nf = None
    ...
    for i in range(steps):
        stop = True
        if nv == None:
            neighs = [v] + list(neighbours_h_1(v, neighbours))
        else:
            neighs = list(neighbours_h_1(nv, neighbours))
        while len(neighs) % n_processes != 0:
            if nv == None:
                neighs = [v] + neighs
            else:
                neighs.append(nv)
        for nb_chunk in divide_chunks(neighs, n_processes):
            for v1, fv1 in pool.map(lambda x: (x, f(x)), nb_chunk): 
                print(fv1)
                sys.stdout.flush()
                if nv == None or fv1 < nf:
                    nf = fv1
                    nv = v1
                    stop = False
            if not stop:
                break
        print("Step, ", i)
        print(vector_to_config(bit_unmerge(ba, nv)))
        print(nf)
        sys.stdout.flush()
        if stop:
            break
...
def merged_neighbours(bucket_assignments):
    return lambda i: list(range(1 << (len(bucket_assignments[i]))))

def optimize(mode, initial_params, param_start, param_domain, remerges, train, bucket_size, seed):
    ...
    for step in range(remerges):

        bucket_assignments = distribute_elements(vector_length, bucket_size)
        for b in bucket_assignments:
            print(b)
        sys.stdout.flush()
        merged_vector, value = hill_climb(
            bit_merge(bucket_assignments, vector), 
            merged_neighbours(bucket_assignments),
            lambda v: call_train(train, mode, vector_to_config(bit_unmerge(bucket_assignments, v)), seed),
            20,
            bucket_assignments)

        vector = bit_unmerge(bucket_assignments, merged_vector)

        print("")
        print("Merge: ", step, sep=" ")
        print(vector_to_config(vector))
        print(value)
        print("")
        sys.stdout.flush()

    return (vector_to_config(vector), value)

file = open("benches_klee_3", "r")
...
solver_timeout = 200000

print(len(default_parameters))
print(len(benchmarks))

def measure_time(config, lines):
    res = {"time": 0, "conflicts": 0}
    benchmark, nn1, nn2, sat_res = lines[:-1].split(", ")
    ...
    if z3_process.returncode != 0:
        ...
        return {"time": 1000000000, "conflicts": 1000000000}
    is_correct = False
    for line in z3_result:
        ...
        if len(kv) == 2:
            key, value = kv
            if key == ":conflicts":
                res["conflicts"] = float(value)
            if key == ":total-time":
                res["time"] = float(value[:-1])
    if is_correct == False:
        return {"time": 1000000000, "conflicts": 1000000000}
    return res
...
def train(config, mode, seed):
    ...
    for value in pool.map(lambda x: measure_time(config, x), sampled_benchmarks):
        result[mode].append(value[mode])
        
    return sum(result[mode]) / len(result[mode])

mode = input()
optimize(mode, default_parameters, param_start, param_domain, 4, train, 2, seed)